  ...
  const handleDeleteSelected = React.useCallback(async () => {
    ...
    try {
      ...
      toastAlert.success({
        ...
        description: `${selectedRows.length} service${selectedRows.length > 1 ? "s" : ""} supprimé${selectedRows.length > 1 ? "s" : ""} avec succès.`,
      })
    } catch (error) {
      console.error(error)
      toastAlert.error({
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-1 items-center space-x-2">
        ...
        {hasSelectedRows && onDeleteSelected && (
          <Button variant="destructive" size="sm" onClick={() => setIsDeleteDialogOpen(true)} className="h-8">
            ...
            Supprimer {selectedRows.length} service{selectedRows.length > 1 ? "s" : ""}
          </Button>
        )}
        ...
        <Button variant="default" onClick={() => router.push("/director/services/new")} className="h-8 px-2 lg:px-3">
        ...
        <Button variant="special" onClick={() => router.push("/director/services/import")} className="h-8 px-2 lg:px-3">
          <PlusIcon className="mr-2 h-4 w-4" />
          Importer
        </Button>
      </div>
      ...
      <ConfirmDialog
        ...
        description={`Êtes-vous sûr de vouloir supprimer ${selectedRows.length} service${selectedRows.length > 1 ? "s" : ""} ? Cette action est irréversible.`}
