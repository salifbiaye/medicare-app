...
import { deleteMultipleServicesAction } from "@/actions/service.action"
...
function DataTableContent<TData, TValue>({
  ...
  const handleDeleteSelected = async (selectedRows: TData[]) => {
    try {
      // @ts-ignore
      const ids: string[] = selectedRows.map(service => service.id)
      await deleteMultipleServicesAction(ids)
      return { success: true }
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 text-sm text-muted-foreground">
          {table.getFilteredSelectedRowModel().rows.length > 0 && (
            <p>
              {table.getFilteredSelectedRowModel().rows.length} sur {table.getFilteredRowModel().rows.length} service(s)
              sélectionné(s).
            </p>
          )}
        </div>
