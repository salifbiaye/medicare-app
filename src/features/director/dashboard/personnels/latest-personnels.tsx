...
import { getLatestPersonnelsAction } from "@/actions/user.action"
import { Skeleton } from "@/components/ui/skeleton"

export function LatestPersonnels() {
    ...
    React.useEffect(() => {
        const fetchPersonnels = async () => {
            setIsLoading(true)
            try {
                const result = await getLatestPersonnelsAction(5) // Récupérer les 5 derniers personnels
    ...
    return (
        <Card className="bg-background dark:bg-muted/30">
            <CardHeader className="bg-gray-600 py-6 text-white dark:bg-muted">
                <CardTitle>Derniers Personnels</CardTitle>
                <CardDescription>Les 5 derniers personnels ajoutés</CardDescription>
            </CardHeader>
                    ...
                    {personnels.map((personnel) => (
                        ...
                        >
                            <Avatar className="h-9 w-9 ">
                                <AvatarImage src={personnel.image || ""} alt={personnel.name} />
                                <AvatarFallback className={"bg-gray-500 text-white dark:bg-background/50"}>
                                    {personnel.name.slice(0, 2).toUpperCase()}
                                </AvatarFallback>
                            </Avatar>

                            <div className="ml-4 space-y-1">
                                <p className="text-sm font-medium leading-none">{personnel.name}</p>
                                <p className="text-sm text-muted-foreground">{personnel.email}</p>
                            </div>
                            <div className="ml-auto flex items-center gap-2">
                                <Badge variant={getRoleBadgeVariant(personnel.role)}>
                                    {formatRoleName(personnel.role)}
                                </Badge>
                                <p className="text-sm text-muted-foreground">
                                    {format(new Date(personnel.createdAt), "dd MMM yyyy", { locale: fr })}
                                </p>
                            </div>
                        </div>
                    ))}
