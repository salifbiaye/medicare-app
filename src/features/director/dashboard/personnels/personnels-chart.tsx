...
import { getPersonnelsByDateRangeAction } from "@/actions/user.action"
import { Skeleton } from "@/components/ui/skeleton"

type PersonnelChartData = {
  date: string
  total: number
  doctors: number
  secretaries: number
...
export function PersonnelsChart() {
  const [timeRange, setTimeRange] = React.useState("90d")
  const [chartData, setChartData] = React.useState<PersonnelChartData[]>([])
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await getPersonnelsByDateRangeAction({
          startDate,
          endDate,
        })

        if (result.success && result.data) {
          // Grouper les personnels par date
          const groupedData = result.data.personnel.reduce((acc: { [key: string]: PersonnelChartData }, personnel: Personnel) => {
            const date = format(new Date(personnel.createdAt), "yyyy-MM-dd")
            if (!acc[date]) {
              acc[date] = {
                date,
                total: 0,
            ...
            acc[date].total++
            if (personnel.role === 'DOCTOR' || personnel.role === 'CHIEF_DOCTOR') {
              acc[date].doctors++
            } else if (personnel.role === 'SECRETARY') {
              acc[date].secretaries++
            }
            return acc
          }, {})

          // Convertir en tableau et trier par date
          const sortedData = Object.values(groupedData) as PersonnelChartData[];
      ...
      <Card className={"bg-muted dark:bg-muted/30"}>
        <CardHeader className="flex bg-gray-600 text-white dark:bg-muted items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
          <div className="grid flex-1 gap-1 text-center sm:text-left">
            ...
            <CardDescription>Visualisation du personnel sur la période sélectionnée</CardDescription>
          </div>
          ...
          <ChartContainer config={chartConfig} className="aspect-auto h-[250px] w-full">
            <AreaChart data={chartData}>
              <defs>
                ...
                <linearGradient id="fillSecretaries" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="var(--color-secretaries)" stopOpacity={0.8} />
                  <stop offset="95%" stopColor="var(--color-secretaries)" stopOpacity={0.1} />
                </linearGradient>
              </defs>
              ...
              <Area
                  dataKey="secretaries"
                  type="natural"
                  fill="url(#fillSecretaries)"
                  stroke="var(--color-secretaries)"
                  stackId="a"
              />
              <Area
                  dataKey="doctors"
